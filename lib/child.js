...
Child.prototype.spawn = function (callback) {
	...
	// handler for disconnect and exiting of the worker
	worker.on('exit', self.handleExit);
...
Child.prototype.replace = function (callback) {
	...
	this.spawn(function (err) {
		...
		// remove the listener on the old worker
		oldWorker.removeListener('exit', self.handleExit);
		// attach a temporary listener to attend to the worker dying
		oldWorker.on('exit', function () {
			self.log('Child process: replaced worker exited, worker ID: ' + oldWorker.id);
		});
