...
function Child(script) {
	...
	this.restartTimer = new Exponential(
		{
			startMs: 200,
			maxMs: 120000,
			multiplier: 2
...
Child.prototype.handleExit = function (code, signal) {
	...
	if (this.worker.suicide) {
		...
		if (code !== 0) {
			// exponentially increase restart timer
			this.restartTimer.failed();
			// log the action
			this.log('Attempting to restart in '
				+ this.restartTimer.currentTimeoutMs + 'ms');
