...
Child.prototype.handleExit = function (code, signal) {
	...
	this.log('Child process: worker exit event received, worker ID: ' + this.worker.id);
	...
	if (this.worker.suicide) {
		...
		if (code !== 0) {
			this.log('Child process: attempting to restart worker in '
...
Child.prototype.spawn = function (callback) {
	...
	worker.on('message', function (message) {
		...
		if (message.cmd && message.cmd === 'ready') {
			self.log('Child process: Worker ready message received, worker ID: ' + self.worker.id);
