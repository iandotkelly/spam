...
cluster.on('exit', function (worker, code, signal) {
	...
	for (index = spam.children.length - 1; index >= 0; index--) {

		child = spam.children[index];
...
function handleExit(child, worker, code, signal) {
	...
	if (worker.suicide) {
		...
		if (signal) {
			...
			logMessage('               worker exited with error code: ' + code);
		} else {
			logMessage('               worker exited without error');
		}
...
function setScript(script) {
	...
	if (!cluster.isMaster) {
		throw new Error('a child process cannot run spam.setScript()');
	}
...
function spawn(options, callback) {
	...
	spam.children = spam.children.concat(spawning);
